...
def export2somaworkflow(in_datasets_file_relative_path,
    ...
    in_working_directory: string
        Since all the data (e.g. in_datasets_file_relative_path) should use relative paths,
    ...
    in_nodes: a list of epac.base.WFNode
        All the data (e.g. in_datasets_file_relative_path) should use relative paths, and
        "in_working_directory" contains those data. The nodes can be
        generated by e.g. 'nodes = wf.get_node(regexp="*/Permutations/*")' Once
    ...
    if in_nodes:
        ...
        jobs = [Job(command=[u"epac_mapper",
                             u'--datasets', '"%s"' % (in_datasets_file_relative_path),
...
def run_multi_processes(
    ...
    cmd_jobs = [u'epac_mapper --datasets="%s" --keysfile="%s"'
                % (in_datasets_file_relative_path, jobfile) for jobfile in keysfile_list]
