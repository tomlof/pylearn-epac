...
def pushNodeInList(node,nodes_per_processor_list):
...
def exportNodes4somaworkflow_it(node, numProcessors, nodes_per_processor_list):
    '''Recursive to allocate the nodes for list
    '''
    numProcessors=len(nodes_per_processor_list)
    children_nodes = node.children    
    len_children = len(children_nodes)
    if len_children == 0 :
        nodes_per_processor_list=pushNodeInList(
                                        node, 
                                        nodes_per_processor_list)
        return nodes_per_processor_list
    left = len_children % numProcessors
    if len_children >= numProcessors :
    ...
    if left > 0 :
        for i in range(len_children-left,len_children):
            nodes_per_processor_list=exportNodes4somaworkflow_it(
                                        children_nodes[i], 
                                        numProcessors,
                                        nodes_per_processor_list)
    return nodes_per_processor_list
    
def exportNodes4somaworkflow(node, numProcessors):
    '''export nodes 
    Try to build "numProcessors" queues which contains almost equally number
...
def export2somaworkflow(in_datasets_file, 
    ...
    if in_nodes!= None:
        ...
        if in_tree_root == None:
            raise NoEpacTreeRoot
        
        nodes_per_processor_list=exportNodes4somaworkflow(
                                        in_tree_root,
                                        in_num_cores)
        ...
        for npp_key in nodes_per_processor_list.keys():
            ...
            abs_keysfile=os.path.join(in_working_directory,keysfile)
